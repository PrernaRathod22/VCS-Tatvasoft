...
namespace Data_Access_Layer
{
    public class DALMission
    {
            ...
            try
            {
                clientSideMissionList = _cIDbContext.Missions
                    ...
                    .Select(m => new Missions
                    {
                        ...
                        MissionApproveStatus = _cIDbContext.MissionApplication.Any(ma => ma.MissionId == m.Id && ma.UserId == userId && ma.Status == true && !ma.IsDeleted) ? "Approved" : "Applied",
        ...
        public async Task<string> DeleteMissionApplicationAsync(int id)
        {
            string result = "";
            try
            {
                var missionApplication = await _cIDbContext.MissionApplication.Where(x => x.Id == id).FirstOrDefaultAsync();
                if (missionApplication != null)
                {
            ...
            {
                throw new Exception("Error while deleting mission application.", ex);
            }
            return result;
        }
